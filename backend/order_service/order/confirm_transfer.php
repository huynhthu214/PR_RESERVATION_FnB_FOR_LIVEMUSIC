...
require_once __DIR__ . '/../../customer_service/db.php'; // $conn_customer
require_once __DIR__ . '/../../notification_service/db.php'; // $conn_noti
...
if ($payCheck && $payCheck['PAYMENT_STATUS'] === 'Completed') {
    echo json_encode(['success'=>false,'message'=>'Đơn hàng đã thanh toán trước đó']);
    exit;
}
...
// Ghi log EMAIL_LOG theo thứ tự EL001, EL002...
try {
    $res = $conn_noti->query("SELECT EMAILLOG_ID FROM EMAIL_LOG ORDER BY EMAILLOG_ID DESC LIMIT 1");
    if($res && $res->num_rows>0){
        $lastId = $res->fetch_assoc()['EMAILLOG_ID'];
        $num = intval(substr($lastId,2))+1;
        $emailLogId = 'EL'.str_pad($num,3,'0',STR_PAD_LEFT);
    } else {
        $emailLogId = 'EL001';
    }

    $adminId = null;
    $stmtLog = $conn_noti->prepare("INSERT INTO EMAIL_LOG (EMAILLOG_ID, ADMIN_ID, CUSTOMER_ID, RECIPIENT_EMAIL, SUBJECT, SENT_TIME, STATUS, ERRORMESSAGE) VALUES (?, ?, ?, ?, ?, NOW(), ?, ?)");
