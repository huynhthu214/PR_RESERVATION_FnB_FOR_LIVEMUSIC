...
$userApiUrl = "http://localhost/PR_RESERVATION_FnB_FOR_LIVEMUSIC/backend/customer_service/get_email.php?customer_id=" . urlencode($user_id);
$userResponse = @file_get_contents($userApiUrl);

if ($userResponse === false) {
    echo json_encode(["success" => false, "message" => "Không thể kết nối Customer Service"]);
    exit;
}
...
// --- Lưu OTP mới vào DB ---
$otp_id = uniqid("OTP");
$stmt = $conn->prepare("INSERT INTO OTP (OTP_ID, CUSTOMER_ID, PAYMENT_ID, CODE, STATUS, IS_USED, CREATED_AT, EXPIRES_AT) 
                        VALUES (?, ?, ?, ?, 'ACTIVE', 0, NOW(), ?)");
$stmt->bind_param("sssss", $otp_id, $payment_id, $customer_id, $otp, $expires_at);
