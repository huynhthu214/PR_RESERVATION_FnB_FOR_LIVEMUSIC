...
$seats = $input['seats'] ?? [];
$menu = $input['menu'] ?? [];
$paymentMethod = $conn_order->real_escape_string($input['paymentMethod'] ?? 'Chuyển khoản');

$totalMenu = array_reduce($menu, fn($sum,$i)=>$sum+($i['unit_price']*$i['quantity']), 0);
$totalSeat = array_sum(array_column($seats,'price'));
$total = floatval($input['total'] ?? ($totalSeat + $totalMenu));

// ==========================
// 1. Tạo 1 RESERVATION cho tất cả ghế
// ==========================
$sqlLastRes = "SELECT RESERVATION_ID FROM RESERVATIONS ORDER BY RESERVATION_ID DESC LIMIT 1";
$resRes = $conn_reservation->query($sqlLastRes);
...
$seatNumbers = implode(",", array_column($seats,'number'));

$stmtRes = $conn_reservation->prepare("
    INSERT INTO RESERVATIONS
    (RESERVATION_ID, EVENT_ID, CUSTOMER_ID, RESERVATION_TIME, STATUS, TOTAL_AMOUNT, PRICE, SEAT_NUMBER)
    VALUES (?, ?, ?, NOW(), 'Pending', ?, ?, ?)
");
$stmtRes->bind_param("sssdds", $reservationId, $eventId, $customerId, $totalSeat, $totalSeat, $seatNumbers);
if(!$stmtRes->execute()){
    echo json_encode(["success"=>false,"message"=>"Lỗi tạo reservation: ".$stmtRes->error]);
    exit;
}
